 ...
 * pidfile handling
 */
static const char *	__rpctest_pidfile = NULL;

static void
__rpctest_term_handler(int dummy)
{
	if (__rpctest_pidfile)
		unlink(__rpctest_pidfile);
	__rpctest_pidfile = NULL;
	exit(15);
}

int
rpctest_pidfile_write(const char *path, pid_t pid)
{
	FILE *fp;

	if (__rpctest_pidfile && strcmp(path, __rpctest_pidfile)) {
		fprintf(stderr, "%s: we've already created a pidfile\n", __func__);
		return -1;
	}

	__rpctest_pidfile = strdup(path);
	signal(SIGINT, __rpctest_term_handler);
...
static pid_t
__rpctest_pidfile_read(const char *path)
{
	FILE *fp;
	char linebuf[128];
	pid_t pid = 0;
...
static pid_t
__rpctest_pidfile_kill(const char *path, int signal)
{
	pid_t pid;

	if ((pid = __rpctest_pidfile_read(path)) <= 0)
		return 0;
...
int
rpctest_pidfile_check(const char *path)
{
	return __rpctest_pidfile_kill(path, 0);
}

int
rpctest_pidfile_kill(const char *path)
{
	return __rpctest_pidfile_kill(path, SIGTERM);
}
